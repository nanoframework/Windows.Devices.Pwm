...
else
{
    ...
    git add "source\Windows.Devices.Pwm\Properties\AssemblyInfo.cs"
    ...
    # new branch name
    $newBranch = "$env:APPVEYOR_REPO_BRANCH-nfbot/update-version/Windows.Devices.Pwm/$env:GitVersion_NuGetVersionV2"
    ...
    $assemblyFiles = (Get-ChildItem -Path ".\*" -Include "win_dev_pwm_native.cpp" -Recurse)
    ...
    {
        $commitMessage = "Update Windows.Devices.Pwm version to $env:GitVersion_AssemblySemFileVer"
